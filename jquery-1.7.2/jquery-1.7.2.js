...
// 我们检查JavaScript引擎是使用一些优化,
// optimization where it does not always call our comparision
// 它并不总是调用我们的比较函数。如果是这样的话,抛弃hasDuplicate值。
...
[0, 0].sort(function() {
	 ...
	 * @param context  {HTMLCollection} 依赖查询的dom
	 * @param results  {HTMLCollection} 查询结果集
	 * @param seed     {boolean}
	 * @returns {*}
	 * @constructor
	 */
	...
	while ( expr && set.length ) {
		for ( type in Expr.filter ) {
			if ( (match = Expr.leftMatch[ type ].exec( expr )) != null && match[2] ) {
				...
				left = match[1];// 继续分离出前置

				anyFound = false;

				match.splice(1,1);// 在match里面删除前置
